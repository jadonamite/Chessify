...
const getToastStyles = (cfg: Config, isBottom: boolean) => ({
  container: {
...
export default function CenterToast() {
  ...
  const styles = getToastStyles(cfg, isBottom)
  return (
    <AnimatePresence>
      {toast && cfg && (
        <div className="fixed inset-0 pointer-events-none z-[9999] flex justify-center" style={styles.container}>
          <motion.div key={toast.type + toast.message} initial={styles.toast.initial} animate={styles.toast.animate} exit={styles.toast.exit} transition={{ type: 'spring', damping: 26, stiffness: 340 }} className="pointer-events-auto mx-4 w-full max-w-sm">
            <div className="rounded-2xl border backdrop-blur-xl shadow-2xl overflow-hidden flex items-center gap-4 px-5 py-4" style={styles.content}>
              {/* Icon bubble */}
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg font-black shrink-0" style={styles.icon}>
                {cfg.icon}
              </div>
              {/* Text */}
              <div className="flex flex-col flex-1 min-w-0">
                <span className="text-[9px] font-black tracking-[0.25em] uppercase mb-0.5" style={styles.label}>
                  {cfg.label}
                </span>
