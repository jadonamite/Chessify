...
// Extracted helper function to process game data and extract unique addresses
function extractAddresses(games: any[]): Set<string> {
...
// Extracted helper function to calculate leaderboard entries from stats results
function calculateLeaderboard(addresses: string[], statsResults: any[]): LeaderboardEntry[] {
...
export function useStacksLeaderboard(enabled = true) {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const addressSet = extractAddresses(games)
