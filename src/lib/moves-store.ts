...
function withRedis<T>(fn: (redis: Redis) => Promise<T>): Promise<T> {
  return fn(getRedis())
}
...
export async function registerActiveGame(chain: Chain, gameId: number): Promise<void> {
  await withRedis((redis) => redis.sadd(activeKey(chain), String(gameId)))
}
...
export async function unregisterActiveGame(chain: Chain, gameId: number): Promise<void> {
  await withRedis((redis) => redis.srem(activeKey(chain), String(gameId)))
}
...
export async function getActiveGameIds(chain: Chain): Promise<number[]> {
  const raw = await withRedis((redis) => redis.smembers(activeKey(chain)))
  return raw
    .map((m) => Number(m))
