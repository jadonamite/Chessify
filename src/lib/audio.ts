...
export function startAmbient(_ctx?: AudioContext) {
  if (activeTrack === 'landing') return stopTrack('game', 800)
  startTrack('landing', 0.55)
}
export function startGameTrack(_ctx?: AudioContext) {
  if (activeTrack === 'game') return stopTrack('landing', 800)
  startTrack('game', 0.5)
}
...
function createMoveSoundSource(ctx: AudioContext, isOpponent: boolean): void {
...
export function playMoveSound(ctx: AudioContext, isOpponent = false) {
  createMoveSoundSource(ctx, isOpponent)
}
