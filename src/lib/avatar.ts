...
function generateSvgString(addr: string, size: number, c1: string, c2: string, piece: string, id: string): string {
...
export function generateAvatarSvg(address: string, size = 100): string {
  ...
  const id = addr.replace(/[^a-z0-9]/g, '').slice(2, 10)
  return generateSvgString(addr, size, c1, c2, piece, id)
}
