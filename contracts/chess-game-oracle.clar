;; ============================================================
;; chess-game-oracle.clar
;; On-chain Chess Protocol -- Oracle settlement model (Stacks)
;;
;; Stacks counterpart of the converged EVM oracle ChessGame. Mirrors
;; chess-game-v2.clar but moves result declaration server-side:
;;
...
;;     chess.js and submits the terminal result here. Funds can only ever go to
;;     white / black / split -- never to the oracle.
;;   - reclaim-expired is an oracle-independent backstop: after EXPIRY-BLOCKS
;;     either participant recovers the escrow (split refund).
;;   - submit-move / report-win / claim-timeout are REMOVED (no on-chain move
;;     clock; the relay + oracle replace them). The game struct drops
;;     turn / move-count / last-move-block to match the EVM struct.
;;
;; !! NOT DEPLOYED. Deployed Stacks game (chess-game) still runs the v2
;;    player-submitted model. Deploy + set-oracle before wiring the frontend.
;;
;; GAME LIFECYCLE:
