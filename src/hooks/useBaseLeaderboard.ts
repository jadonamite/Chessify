...
export function useBaseLeaderboard(enabled = true) {
  ...
  const calculateLeaderboard = (gameResults: any[], statsResults: any[]) => {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      // tuple { wins, losses, draws, rating, gamesPlayed }
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const statsResults = await publicClient.multicall({
        contracts: Array.from(new Set(gameResults.flatMap((r) => r.status === 'success' ? [(r.result as any).white, (r.result as any).black] : []))).filter((a) => a !== ZERO).map((addr) => ({
          address: BASE_CONTRACTS.game as `0x${string}`,
          abi: BASE_CHESS_GAME_ABI as Abi,
      ...
      const leaderboard = calculateLeaderboard(gameResults, statsResults)
