...
import { CELO_CONTRACTS, BASE_CONTRACTS, STACKS_CONTRACTS, } from '@/config/contracts';
...
async function readEvmGame(chain: 'celo' | 'base', gameId: number): Promise<OnchainGame> {
  ...
  const g = await client.readContract({
    ...
    args: [BigInt(gameId)],
  }) as { white: string; black: string; status: number };
  return {
    white: g.white,
    black: g.black,
...
export async function getOnchainGame(chain: Chain, gameId: number): Promise<OnchainGame> {
  if (chain === 'celo') return readEvmGame('celo', gameId);
  if (chain === 'base') return readEvmGame('base', gameId);
