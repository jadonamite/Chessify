...
export async function POST(req: NextRequest) {
  ...
  if (!Array.isArray(body.addresses)) return NextResponse.json({ error: 'addresses array required' }, { status: 400 });
  if (body.addresses.length === 0) return NextResponse.json({ error: 'at least one address required' }, { status: 400 });
