...
export async function POST(req: NextRequest) { 
  ...
  if (displayName.length > 30) return NextResponse.json({ error: 'displayName max 30 characters' }, { status: 400 }) 
  if (bio.length > 120) return NextResponse.json({ error: 'bio max 120 characters' }, { status: 400 }) 
