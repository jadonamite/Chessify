...
const getPieceAnimation = (isInfinite: boolean, progress: number | undefined) => {
  if (isInfinite) {
    return {
      left: ['-20%', '120%'],
      width: ['20%', '40%', '20%']
...
const getPieceTransition = (isInfinite: boolean) => {
  if (isInfinite) {
...
const getFloatSpeed = (isInfinite: boolean) => (isInfinite ? 3 : 2);
const getFloatIntensity = (isInfinite: boolean) => (isInfinite ? 2 : 1);
      ...
      <div className="w-64 h-64 relative">
        <Canvas camera={{ position: [0, 0, 5], fov: 40 }} gl={{ alpha: true }}>
          <Suspense fallback={null}>
            ...
            <group position={getPiecePosition(isInfinite, progress)}>
              <Pawn
                color="#00ccff"
                emissive="#00ccff"
                emissiveIntensity={0.8}
                floatSpeed={getFloatSpeed(isInfinite)}
                floatIntensity={getFloatIntensity(isInfinite)}
              />
            </group>
          </Suspense>
        </Canvas>
      ...
      <div className="w-full max-w-sm flex flex-col items-center gap-4">
        <div className="w-full h-[2px] bg-white/5 relative overflow-hidden rounded-full border border-white/10">
          <motion.div
            ...
            initial={{ width: 0 }}
            animate={getPieceAnimation(isInfinite, progress)}
            transition={getPieceTransition(isInfinite)}
          />
        </div>
        {/* ── STATUS TEXT ── */}
