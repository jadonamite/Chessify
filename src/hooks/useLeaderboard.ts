...
const processGameResults = async (
  ...
  if (gameNonce === 0n) return []

  const ids = Array.from({ length: Number(gameNonce) }, (_, i) => BigInt(i + 1))
...
export function useLeaderboard(enabled = true) {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const leaderboard = await processGameResults(publicClient, gameNonce, enabled)
