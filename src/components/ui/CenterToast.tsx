...
const getToastStyle = (cfg: Config, isBottom: boolean) => ({
  alignItems: isBottom ? 'flex-end' : 'center',
  paddingBottom: isBottom ? '1.5rem' : 0,
})
const getToastAnimation = (cfg: Config, isBottom: boolean) => ({
  initial: { opacity: 0, y: isBottom ? 24 : 0, scale: isBottom ? 0.97 : 0.9 },
  animate: { opacity: 1, y: 0, scale: 1 },
  exit: { opacity: 0, y: isBottom ? 12 : -12, scale: 0.95 },
})
const getToastContentStyle = (cfg: Config) => ({
  background: cfg.bg,
...
export default function CenterToast() {
  ...
  return (
    <AnimatePresence>
      {toast && cfg && (
        <div className="fixed inset-0 pointer-events-none z-[9999] flex justify-center" style={getToastStyle(cfg, isBottom)}>
          <motion.div key={toast.type + toast.message} initial={getToastAnimation(cfg, isBottom).initial} animate={getToastAnimation(cfg, isBottom).animate} exit={getToastAnimation(cfg, isBottom).exit} transition={{ type: 'spring', damping: 26, stiffness: 340 }} className="pointer-events-auto mx-4 w-full max-w-sm">
            <div className="rounded-2xl border backdrop-blur-xl shadow-2xl overflow-hidden flex items-center gap-4 px-5 py-4" style={getToastContentStyle(cfg)}>
              {/* Icon bubble */}
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg font-black shrink-0" style={{ background: `${cfg.glow}`, border: `1px solid ${cfg.border}`, color: cfg.labelColor }}>
                {cfg.icon}
              </div>
              {/* Text */}
              <div className="flex flex-col flex-1 min-w-0">
                <span className="text-[9px] font-black tracking-[0.25em] uppercase mb-0.5" style={{ color: cfg.labelColor, fontFamily: 'var(--fd)' }}>
                  {cfg.label}
                </span>
              ...
              {/* Dismiss */}
              <button onClick={hideToast} className="shrink-0 p-1.5 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/5 transition-colors" aria-label="Dismiss">
