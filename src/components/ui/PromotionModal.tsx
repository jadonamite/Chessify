...
const handleKeyboardEvent = (e: KeyboardEvent, open: boolean, onSelect: (piece: PromotionPiece) => void, onCancel: () => void) => {
  if (!open) return;
...
export default function PromotionModal({ open, color, onSelect, onCancel }: PromotionModalProps) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => handleKeyboardEvent(e, open, onSelect, onCancel);
