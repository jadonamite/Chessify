...
///
/// ⚠️ UPGRADE TARGET — the deployed Base contract still runs the legacy player-submitted
///    `reportWin` model. This oracle source is the next deploy (mirrors the live
///    playchessify oracle game). Do NOT point the frontend at it until redeployed
///    and `setOracle` is called. See README "Migration status".
///
/// DEPLOYMENT ORDER:
///   1. Deploy ChessToken
...
contract ChessGame is ReentrancyGuard, Ownable {
    ...
    uint256 public constant EXPIRY_BLOCKS = 43_200;     // ~1 day on Base (~2s blocks) — reclaim backstop
