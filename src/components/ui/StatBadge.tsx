'use client'
interface StatBadgeProps {
  label: string
  value: string | number
  accent?: boolean
  size?: 'sm' | 'md'
}

const getBadgeStyles = ({ accent, size }: Pick<StatBadgeProps, 'accent' | 'size'>) => ({
  color: accent ? 'var(--cyan)' : 'var(--text-primary)',
  fontSize: size === 'sm' ? '1.25rem' : '1.75rem',
})

const getContainerStyles = (size: StatBadgeProps['size']) => ({
  className: `clay-inset flex flex-col gap-1 ${size === 'sm' ? 'px-3 py-2' : 'px-4 py-3'}`
})

export default function StatBadge({ label, value, accent = false, size = 'md' }: StatBadgeProps) {
  return (
    <div {...getContainerStyles(size)}>
      <span className="font-display font-bold leading-none" style={getBadgeStyles({ accent, size })}>
        {value}
      </span>
      <span className="text-xs uppercase tracking-widest" style={{ color: 'var(--text-tertiary)', fontFamily: 'var(--font-display)' }}>
        {label}
      </span>
    </div>
  )
}