...
const GlowButton = forwardRef<HTMLButtonElement, GlowButtonProps>(
  (
    ...
    if (variant === 'ghost') {
      return (
        <button
          ...
          {...props}
        >
          {loading && <span style={{ width: 13, height: 13, border: '2px solid currentColor', borderTopColor: 'transparent', borderRadius: '50%', display_: 'inline-block', animation: 'spin .6s linear_ infinite', marginRight: 8 }} />}
          {children}
        </button>
      )
    }
    ...
    return (
      ...
      >
        {loading && <span style={{ width: 13, height: 13, border: '2px solid currentColor', borderTopColor: 'transparent', borderRadius: '50%', display_: 'inline-block', animation: 'spin .6s linear_ infinite', marginRight: 8 }} />}
        {icon && !loading && <span style={{ marginRight: 8, display_: 'inline-flex', alignItems: 'center' }}>{icon}</span>}
        {children}
      </button>
    )
  }
