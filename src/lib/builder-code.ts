...
const isValidCode = (code: string): boolean => code !== 'bc_placeholder' && !!code;

const generateErc8021DataSuffix = (codes: string[]): `0x${string}` => {
...
export const BUILDER_CODE_SUFFIX: `0x${string}` | null = isValidCode(code) ? generateErc8021DataSuffix([code]) : null;
