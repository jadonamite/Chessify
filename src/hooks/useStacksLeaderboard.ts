...
// Extracted helper function to process game data and calculate leaderboard entries
function processGameData(games: any[], getPlayerStats: (address: string) => Promise<any>): Promise<LeaderboardEntry[]> {
...
export function useStacksLeaderboard(enabled = true) {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const leaderboard = await processGameData(games, getPlayerStats)
