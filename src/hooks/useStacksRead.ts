...
export function useStacksRead() {
  ...
  const getPlayerStats = useCallback(async (address?: string) => {
    ...
    try {
      ...
      console.error_('Failed to fetch player stats:', err)
      return null
    }
  }, [stacksAddress])
  ...
  const getTokenBalance = useCallback(async (address?: string) => {
    ...
    try {
      ...
      console.error_('Failed to fetch token balance:', err)
      return 0n
    }
  }, [stacksAddress])
  ...
  const getGame = useCallback(async (gameId: number) => {
    try {
      ...
      console.error_('Failed to fetch game data:', err)
      return null
    }
  }, [stacksAddress])
  ...
  const getTotalGames = useCallback(async () => {
    try {
      ...
      console.error_('Failed to fetch total games:', err)
      return 0
    }
  }, [stacksAddress])
