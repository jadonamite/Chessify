...
const extractGameInfo = (
  ...
  else if (statusIdx === 4 || Number(g.result) === 3) result = 'draw'
  else if (statusIdx === 2) {
    if (Number(g.result) === 1) result = role === 'white' ? 'win' : 'loss'
    else if (Number(g.result) === 2) result = role === 'black' ? 'win' : 'loss'
    else result = 'active'
  }
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<PlayerHistoryItem[]> => {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const gameInfo = extractGameInfo(me, g, BigInt(start + i))
        if (gameInfo) items.push(gameInfo)
      }
