...
export default function LobbyContent() {
  ...
  const {
    data: celoBalance,
    isRefetching: celoBalRefetching,
    refetch: refetchCelo,
  } = useReadContract({
    ...
    query: { enabled: activeChain === 'celo' && !!celoAddress, refetchInterval: 5000, staleTime: 0 }
  })
  ...
  const {
    data: baseBalance,
    isRefetching: baseBalRefetching,
    refetch: refetchBase,
  } = useReadContract({
    ...
    args: [celoAddress as `0x${string}`],
    query: { enabled: activeChain === 'base' && !!celoAddress, refetchInterval: 5000, staleTime: 0 }
  })
  ...
  const isBalanceUpdating =
    activeChain === 'celo' ? celoBalRefetching
    : activeChain === 'base' ? baseBalRefetching
    : false

  const refetchBalance = () => {
    if (activeChain === 'celo') refetchCelo()
    else if (activeChain === 'base') refetchBase()
    else if (activeChain === 'stacks') getStacksBalance().then(b => setBalance((Number(b) / Math.pow(10, TOKEN_DECIMALS)).toFixed(2)))
  }
    ...
    if (activeChain === 'stacks' && stacksAddress) {
      ...
      getStacksStats(stacksAddress).then(s => {
        if (s) {
          ...
          setDraws(Number(s.draws?.value ?? 0))
        }
      })
      ...
      if (baseStats) {
        const s = baseStats as { wins: bigint; losses: bigint; draws: bigint; rating: bigint; gamesPlayed: bigint }
        setRating(Number(s.rating))
        setWins(Number(s.wins))
        setLosses(Number(s.losses))
        setDraws(Number(s.draws))
      }
    }
  ...
  const { streak, isLoading: streakLoading } = useStreak(activeAddress)

  // Daily nudge — a 0-streak player who lands on the lobby gets the motivational
  // streak prompt. Strictly once per UTC day.
  const nudgeFired = useRef(false)
  useEffect(() => {
    if (nudgeFired.current) return
    if (!activeAddress || streakLoading || streak.current > 0) return
  ...
  }, [activeAddress, streakLoading, streak.current, streak.longest])

  const connected = isConnected || isStacksConnected
  const showClaimBanner = connected && !!activeAddress && myProfile === null
  ...
  useEffect(() => {
    // Not connected on either chain — silently redirect to landing (no gate card)
  ...
  // Authenticated but the active-chain address isn't resolved yet
  if (!activeAddress) {
    return (
                  ...
                  {(() => {
                    ...
                    return (
                      <TrapButton
                        ...
                        onClick={() => handleAction(() => router.push('/app/train'))}
                      ...
                      <div className="rounded-2xl border border-white/5 bg-black/40 hover:bg-black/60 hover:border-white/10 transition-colors active:border-[var(--c)]/30">
                        <div className="p-4 md:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 md:gap-5">
                          ...
                          <div className="flex items-center justify-between w-full sm:w-auto sm:gap-5 border-t sm:border-t-0 border-white/5 pt-3 sm:pt-0 shrink-0">
                            ...
                            <GlowButton
                              size="sm"
                              onClick={() => handleAction(() => handleJoinGame(game.id, game.wager))}
                              disabled={isPending}
            ...
            <PlayCard size="rail" tone="candy" accent="var(--c)">
              ...
              <div className="p-6 md:p-7 flex flex-col gap-3 relative z-10">
                ...
                <p className="text-[13px] text-gray-400 leading-relaxed">
                  Claim free testnet tokens to start playing on {activeChain}.
                </p>
