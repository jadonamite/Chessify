...
const convertCodesToHex = (codes: string[]): string => {
  const codesStr = codes.join(',');
  const codesBytes = Buffer.from(codesStr, 'ascii');
  return codesBytes.toString('hex');
}

const constructErc8021DataSuffix = (codesHex: string): `0x${string}` => {
  const codesLengthHex = Buffer.from(codesHex, 'hex').length.toString(16).padStart(2, '0');
  return `0x${codesHex}${codesLengthHex}00${ERC8021_SUFFIX}`;
}

export function toErc8021DataSuffix(codes: string[]): `0x${string}` {
  const codesHex = convertCodesToHex(codes);
  return constructErc8021DataSuffix(codesHex);
}
