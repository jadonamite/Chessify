...
import { normalizeAddress } from './profile-address'
...
// ── Key builders ─────────────────────────────────────────────────────────────
// Addresses are normalised per-chain (EVM lowercased, Stacks preserved).
// Usernames are always lowercase.
const K = {
  addr: (a: string) => `chess:profile:addr:${normalizeAddress(a)}`,
  name: (n: string) => `chess:profile:name:${n.toLowerCase()}`,
  namelock: (a: string) => `chess:profile:namelock:${normalizeAddress(a)}`,
...
export function validateUsername(raw: string): { ok: boolean; reason?: string } {
  ...
  if (name.length < 3 || name.length > 20) return { ok: false, reason: 'Username must be 3–20 characters' }
  if (!USERNAME_RE.test(name)) return { ok: false, reason: 'Only lowercase letters, numbers, and hyphens. No leading/trailing hyphens.' }
  if (name.includes('--')) return { ok: false, reason: 'No consecutive hyphens' }
...
export async function claimProfile(profile: ChessProfile): Promise<{ ok: boolean; reason?: string }> {
  ...
  const reserved = await redis.setnx(K.name(name), addr)
  ...
  const finalProfile: ChessProfile = { ...profile, address: addr, og: total <= 100 }
...
export async function updateProfile(
  ...
  const validation = validateProfileUpdates(existing, updates, now, THIRTY_DAYS)
  if (!validation.ok) return validation
  const updated: ChessProfile = { ...existing, ...updates, updatedAt: now }
...
export async function getBatchProfiles(
  ...
  const redis = getRedis()
  const keys = addresses.map((a) => K.addr(a))
...
export async function getRecentProfiles(limit = 10): Promise<ChessProfile[]> {
  ...
  return (addresses as string[]).map((a) => batch[normalizeAddress(a)]).filter(Boolean) as ChessProfile[]
}
function validateProfileUpdates(
  existing: ChessProfile,
  updates: Partial<Pick<ChessProfile, 'username' | 'displayName' | 'bio'>>,
  now: number,
  THIRTY_DAYS: number,
  ...
  if (updates.username && updates.username.toLowerCase() !== existing.username.toLowerCase()) {
    ...
    const redis = getRedis()
    const reserved = redis.setnx(K.name(newName), existing.address)
    if (!reserved) return { ok: false, reason: 'Username already taken' }
    // Release old name
    redis.del(K.name(existing.username.toLowerCase()))
  }
  return { ok: true }
}
