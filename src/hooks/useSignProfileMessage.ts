...
const signMessageWithWagmi = async (signMessageAsync: any, message: string): Promise<SignedMessage> => {
  const signature = await signMessageAsync({ message })
  return { signature }
}

const signMessageWithStacks = async (userSession: any, message: string): Promise<SignedMessage> => {
...
export function useSignProfileMessage() {
  ...
  return useCallback(async (message: string): Promise<SignedMessage> => {
    if (activeChain === 'stacks') {
      return signMessageWithStacks(userSession, message)
    }
    return signMessageWithWagmi(signMessageAsync, message)
