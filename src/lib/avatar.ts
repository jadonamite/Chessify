...
function deriveColors(address: string): [string, string] {
...
function generateSvg(address: string, size: number, c1: string, c2: string): string {
  const piece = PIECES[addrByte(address, 7) % 6]
  const id = address.replace(/[^a-z0-9]/g, '').slice(2, 10)
  // unique gradient id per address
...
export function generateAvatarSvg(address: string, size = 100): string {
  const [c1, c2] = deriveColors(address)
  return generateSvg(address, size, c1, c2)
}
