...
// ─── component ─────────────────────────────────────────────────────────────
//
// Multi-chain game screen for Base + Stacks. Celo runs the redesigned, decomposed
// GameClientCelo (oracle-settled). This container keeps the proven legacy
// player-submitted flow for the chains that still use it — Base (reportWin /
// settleDraw) and Stacks (oracle-settle via /api + Clarity reads). Dispatched by
// activeChain in GameClient.tsx, so the Celo branches are intentionally gone here.

export default function GameClientMultichain() {
  ...
  const handleReportWin = async () => {
    await withTx(async () => {
      ...
      // guaranteed fallback). Base still runs the legacy player-submitted reportWin.
      if (activeChain === 'stacks') await fetch(`/api/games/stacks/${gameId}/settle`, { method: 'POST' })
      else await reportBaseWin(gameId)
    })
  }
  ...
  // Timeout: Stacks has no live on-chain read yet — show the claim (reclaim-expired
  // backstop) when the game is active and it's not my turn. Base has no claimTimeout.
  const canClaimTimeoutNow = !isBotGame && activeChain === 'stacks'
    && gameIsActive && isParticipant && !isMyTurn
  ...
  // Per-move signing (Base silent-signs via EVM; Stacks relays unsigned — see useMoveSigner).
    ...
    for (const m of relayMoves) {
      ...
      if (gameData) {
        ...
        if (normalize(m.player) !== normalize(expectedPlayer)) {
          console.error('[GameClientMultichain] relay player mismatch — aborting replay', {
      ...
      try {
        ...
        console.error('[GameClientMultichain] relay move rejected by chess.js — aborting replay to avoid corrupted state', { move: m })
        return
      }
  ...
  const executeMove = useCallback((sourceSquare: string, targetSquare: string, promotion?: PromotionPiece): boolean => {
    try {
      ...
      // authoritative state if the POST is rejected (409 conflict). Base signs
      // the move (silent EVM); Stacks relays unsigned (signMove returns null).
      if (!isBotGame) {
        const player = activeChain === 'stacks' ? (stacksAddress ?? '') : (celoAddress ?? '')
        void relaySubmitMove(move.san, player, next.fen(), moveSigner.signMove).then((ok) => {
          if (!ok) {
            console.warn('[GameClientMultichain] relay rejected move — resyncing from authoritative state', { san: move.san })
            setStatusModalType('invalid_move')
      ...
      console.error('[GameClientMultichain] executeMove failed:', e)
      setStatusModalType('invalid_move')
      if (game.inCheck()) {
        setStatusModalMessage('Invalid move: Your King is in check!')
      } else {
  ...
  const withTx = useCallback(async (fn: () => Promise<unknown> | undefined) => {
    if (txPending) return
    setTxPending(true)
    try { await fn() } catch (e) { console.error('[GameClientMultichain] tx error:', e) } finally { setTxPending(false) }
  }, [txPending])

  const handleAcceptDraw = async () => {
    await withTx(async () => {
      if (activeChain === 'stacks') await acceptStacksDraw(gameId)
      else await settleBaseDraw(gameId)  // base step 2: accept
    })
  }
  ...
  const handleProposeDraw = async () => {
    await withTx(async () => {
      ...
      else await settleBaseDraw(gameId)  // base step 1: propose
    })
  }
...
// TODO: optimize for large datasets
  ...
  const handleJoinMatch = async () => {
    ...
    await withTx(async () => {
      ...
      console.info('[GameClientMultichain] joining game from page', { gameId, wagerInChess })
  ...
  // runs from a clean slate (avoids stale data leaking into a different lookup).
  useEffect(() => {
    setGameData(null)
    setLoadError(false)
    setStacksDataLoaded(false)
          ...
          {loadError && (
            <>
              ...
              <GlowButton
                ...
                size="sm"
                parallelogram
                onClick={() => setActiveChain(activeChain === 'stacks' ? 'base' : 'stacks')}
              >
                SEARCH ON {activeChain === 'stacks' ? 'BASE' : 'STACKS'}
              </GlowButton>
            </>
          )}
          <Link href="/app/lobby">
              ...
              {canJoinFromPage ? (
                ...
                <ClayCard className="p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Timeout claim — Stacks expiry backstop only */}
                    {canClaimTimeoutNow && (
                      <GlowButton
                        variant="ghost"
                        fullWidth
