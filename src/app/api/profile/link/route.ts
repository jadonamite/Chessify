...
export async function POST(req: NextRequest) {
  ...
  if (!pEoa && !pSmart) return NextResponse.json({ ok: true, linked: 'none' })
  if (pSmart && !pEoa) {
    await linkProfileAlias(eoa, smart)
    return NextResponse.json({ ok: true, linked: 'eoa->smart' })
  }
  if (pEoa && !pSmart) {
    await linkProfileAlias(smart, eoa)
    return NextResponse.json({ ok: true, linked: 'smart->eoa' })
  }
  return NextResponse.json({ ok: true, linked: 'none' })
}